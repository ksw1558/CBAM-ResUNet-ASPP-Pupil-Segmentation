...
from nets.attention_unet import AttentionGateUnet
...
if __name__ == "__main__":
    ...
    weight_decay = 0
    lr_decay_type = 'cos'
    # Only keep best_epoch_weights.pth, last_epoch_weights.pth and final_weights.pth.
    save_period = 0
    save_dir = 'logs'

    eval_flag = True
    eval_period = 5  # 每 5 代评估一次

    # 数据集路径
    VOCdevkit_path = os.path.join(root_path, 'VOCdevkit')

    # ------------------------------------------------------------------#
    #   损失函数：瞳孔占比小，必须开启 Dice Loss
    # ------------------------------------------------------------------#
    dice_loss = True
    focal_loss = False
    ...
    # 实例化模型
    model = AttentionGateUnet(num_classes=num_classes, pretrained=pretrained, backbone=backbone).train()
