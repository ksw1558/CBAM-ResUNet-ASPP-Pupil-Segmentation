...
from nets.unet_plus_plus import UNetPlusPlus  # noqa: E402
...
def evaluate():
    print("=" * 60)
    print("Evaluate Experiment 05: UNet++ on LPW (VOC format)")
    print("=" * 60)

    exp_dir = os.path.join(ROOT_DIR, "05_UNet_PlusPlus_Exp")
    ...
    model = UNetPlusPlus(num_classes=2, pretrained=False, backbone="vgg", deep_supervision=False)
    ...
    for image_id in tqdm(image_ids, desc="Predict"):
        ...
        x = preprocess_image(image_path, input_size=256).to(device)
        with torch.no_grad():
            output = model(x)
